...
## Paso 5.1: Variables de seguridad obligatorias (webhooks)

Asegúrate de configurar también estas variables en el runtime de Cloud Run:

- `SUPABASE_SERVICE_ROLE_KEY` (obligatoria para replay protection persistente)
- `WEBHOOK_REPLAY_REQUIRE_PERSISTENT=true` (fail-closed en producción)
...
gcloud run services update barflow \
  --region us-central1 \
  --set-env-vars WEBHOOK_REPLAY_REQUIRE_PERSISTENT=true
```

> Recomendado: inyectar `SUPABASE_SERVICE_ROLE_KEY` desde Secret Manager, no como texto plano.
...
Validación esperada en `/api/health` para producción segura:

- `security_runtime: pass`
- `security_replay_store: pass`
- `status: healthy`
...
- [ ] `SUPABASE_SERVICE_ROLE_KEY` configurado en runtime
- [ ] `WEBHOOK_REPLAY_REQUIRE_PERSISTENT=true` configurado en runtime
- [ ] `security_runtime=pass` en `/api/health`
- [ ] `security_replay_store=pass` en `/api/health`
