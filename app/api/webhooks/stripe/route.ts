...
async function findEstablishmentBySubscription(
  supabase: any,
...
function getPlanTypeFromPriceId(priceId: string): string {
  const starterMonthlyId =
    process.env.NEXT_PUBLIC_STRIPE_STARTER_MONTHLY_PRICE_ID;
  const starterYearlyId =
    process.env.NEXT_PUBLIC_STRIPE_STARTER_YEARLY_PRICE_ID;
  const businessMonthlyId =
    process.env.NEXT_PUBLIC_STRIPE_BUSINESS_MONTHLY_PRICE_ID;
  const businessYearlyId =
    process.env.NEXT_PUBLIC_STRIPE_BUSINESS_YEARLY_PRICE_ID;

  if (priceId === starterMonthlyId) return "starter_monthly";
  if (priceId === starterYearlyId) return "starter_yearly";
  if (priceId === businessMonthlyId) return "business_monthly";
  if (priceId === businessYearlyId) return "business_yearly";
  return "starter_monthly"; // default
}
    ...
    switch (event.type) {
      ...
      case "checkout.session.completed": {
        ...
        if (establishmentId && subscriptionId) {
          ...
          console.log(
            `✅ Subscription activated: ${establishmentId} (${planType})`,
          );
          ...
          if (userId) {
            ...
            if (email) {
              const planNames: Record<string, string> = {
                ...
                business_monthly: "Business (Mensual)",
      ...
      case "invoice.payment_succeeded": {
        ...
        if (subscriptionId) {
          const establishment = await findEstablishmentBySubscription(
            supabase,
            subscriptionId,
          );
      ...
      case "invoice.payment_failed": {
        ...
        if (subscriptionId) {
          ...
          if (establishment) {
            ...
            console.log(
              `❌ Payment failed (attempt ${attemptCount}): ${establishment.id}`,
            );
      ...
      case "charge.dispute.created": {
        ...
        console.log(
          `⚖️ Dispute created: ${dispute.id} - Amount: ${dispute.amount}`,
        );
