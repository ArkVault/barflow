...
export default function AccountContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    "profile" | "security" | "subscription" | "connections" | "settings"
  >("profile");
  ...
  // Settings (Configuración) state
  const [settingsName, setSettingsName] = useState("");
  const [settingsTaxRate, setSettingsTaxRate] = useState(16);
  const [settingsLowStock, setSettingsLowStock] = useState(20);
  const [settingsAutoUpdate, setSettingsAutoUpdate] = useState(true);
  const [settingsExpiryAlerts, setSettingsExpiryAlerts] = useState(true);
  const [settingsAnalysisDays, setSettingsAnalysisDays] = useState(30);
  const [settingsAutoProjections, setSettingsAutoProjections] = useState(true);
  const [settingsPurchaseRecs, setSettingsPurchaseRecs] = useState(true);
  ...
  const fetchSettings = async () => {
    ...
    if (est) {
      setSettingsName(est.name || "");
      setSettingsTaxRate(est.tax_rate ?? 16);
    }
  };
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const result = await updateEstablishmentSettings(establishmentId, {
        name: settingsName,
      ...
      if (result.success) {
        ...
        // Also update profile name to stay in sync
        setProfile((prev) => ({
          ...prev,
          establishment_name: settingsName,
        }));
      } else {
        toast.error(
          result.error ||
            (language === "es" ? "Error al guardar" : "Error saving settings"),
        );
      }
    } catch {
      toast.error(
  ...
  const handleConnectOpenTable = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("opentable_integrations").upsert(
        {
  ...
  const handleDisconnectOpenTable = async () => {
    try {
      ...
      toast.success(
        language === "es" ? "OpenTable desconectado" : "OpenTable disconnected",
      );
    } catch (error) {
      console.error("Error disconnecting OpenTable:", error);
      toast.error(
        language === "es" ? "Error al desconectar" : "Error disconnecting",
      );
    }
  };
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <div className="flex gap-6 border-b border-border">
        ...
        <button
          ...
          className={`relative pb-4 text-sm font-medium transition-colors ${activeTab === "settings" ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}
        >
          {language === "es" ? "Configuración" : "Settings"}
          {activeTab === "settings" && (
            <span className="absolute bottom-0 left-0 h-0.5 w-full bg-cyan-500 shadow-[0_0_12px_rgba(6,182,212,0.6)]" />
          )}
        </button>
      </div>
          ...
          <div className="space-y-8">
            ...
            {currentPlanIsTopTier && (
              <div className="neumorphic rounded-2xl p-8 text-center space-y-3">
                ...
                <p className="text-muted-foreground">
                  {language === "es"
                    ? "Tienes acceso a todas las funcionalidades exclusivas de Stttock."
                    : "You have access to all exclusive Stttock features."}
                </p>
              </div>
            )}
          </div>
            ...
            <div className="neumorphic rounded-2xl p-8 space-y-6">
              ...
              <div className="border border-border rounded-xl p-6 space-y-4 hover:border-orange-500/50 transition-colors">
                <div className="flex items-start justify-between">
                  ...
                  <div
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${openTableConnected ? "bg-green-500/10" : "bg-muted"}`}
                  >
                    <span
                      className={`w-2 h-2 rounded-full ${openTableConnected ? "bg-green-500" : "bg-gray-400"}`}
                    ></span>
                    <span
                      className={`text-xs font-medium ${openTableConnected ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}
                    >
                      {openTableConnected
                        ? language === "es"
                          ? "Conectado"
                ...
                <div className="pt-4 border-t border-border">
                  ...
                  <ul className="space-y-2">
                    {[
                      ...
                      language === "es"
                        ? "Detalles de cliente y reservación"
        ...
        {activeTab === "settings" && (
          <div className="space-y-8">
            ...
            <div className="neumorphic rounded-2xl p-8 space-y-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {language === "es"
                      ? "Nombre, impuestos y preferencias del establecimiento"
                      : "Name, taxes, and establishment preferences"}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {language === "es"
                      ...
                      : "Border zone: 8% · Standard: 16% · Export / exempt: 0%"}
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select className="w-full px-4 py-2 rounded-xl neumorphic-inset bg-background focus:outline-none focus:ring-2 focus:ring-primary text-sm">
            ...
            <div className="neumorphic rounded-2xl p-8 space-y-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {language === "es"
                      ? "Umbrales, alertas y automatización"
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setSettingsLowStock(Number(e.target.value))
                    }
                    min={0}
                    max={100}
                    className="neumorphic-inset"
                  ...
                  <p className="text-xs text-muted-foreground">
                    {language === "es"
                      ...
                      : "Percentage below which stock is considered critical"}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between py-3 border-b border-border">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {language === "es"
                        ? "Deducir insumos al registrar ventas"
                        : "Deduct supplies when recording sales"}
                    </p>
                  </div>
                  <Switch
                    checked={settingsAutoUpdate}
                    onCheckedChange={setSettingsAutoUpdate}
                  />
                </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {language === "es"
                        ...
                        : "Notify about products close to expiration"}
                    </p>
                  </div>
                  <Switch
                    checked={settingsExpiryAlerts}
                    onCheckedChange={setSettingsExpiryAlerts}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="neumorphic rounded-2xl p-8 space-y-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {language === "es"
                      ? "Análisis, proyecciones y recomendaciones"
                      : "Analysis, projections, and recommendations"}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <Input
                    type="number"
                    value={settingsAnalysisDays}
                    onChange={(e) =>
                      setSettingsAnalysisDays(Number(e.target.value))
                    }
                    min={7}
                    max={90}
                    className="neumorphic-inset"
                  ...
                  <p className="text-xs text-muted-foreground">
                    {language === "es"
                      ...
                      : "History days for calculating projections"}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between py-3 border-b border-border">
                  ...
                  <Switch
                    checked={settingsAutoProjections}
                    onCheckedChange={setSettingsAutoProjections}
                  />
                </div>
                ...
                <div className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {language === "es"
                        ? "Sugerencias basadas en consumo"
                        : "Suggestions based on consumption"}
                    </p>
                  </div>
                  <Switch
                    checked={settingsPurchaseRecs}
                    onCheckedChange={setSettingsPurchaseRecs}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-4">
              ...
              >
                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-gray-400 to-gray-600 flex items-center justify-center shadow-inner">
          ...
          <div className="neumorphic rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto animate-in zoom-in-95 fade-in duration-300">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {language === "es"
                    ? "Plan Cadena - Múltiples sucursales"
                    : "Chain Plan - Multiple branches"}
                </p>
              </div>
              <button
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label className="flex items-center gap-2">
                  ...
                  {language === "es"
                    ? "¿Cuántas sucursales necesitas? *"
                    : "How many branches do you need? *"}
                </Label>
                <Input
                  value={quoteForm.branches}
                  onChange={(e) =>
                    setQuoteForm({ ...quoteForm, branches: e.target.value })
                  }
                  placeholder={
                    language === "es"
                      ? "Ej: 3 sucursales actuales, planeamos 5 más"
                      : "E.g: 3 current branches, planning 5 more"
                  }
                />
              </div>
