...
export function TablesTab() {
    ...
    const updated = sections.map((section) => {
      if (section.id === sectionId) {
        const newTable: TableItem = {
          id: `table-${Date.now()}`,
          name:
            language === "es" ? `Mesa ${nextNumber}` : `Table ${nextNumber}`,
  ...
  // Go to orders tab with selected table and optionally a specific account
  const goToOrdersTab = (
    sectionId: string,
    itemId: string,
    type: "table" | "bar",
    ...
    setSelectedAccountForOrder(accountId ?? null);
  ...
  useEffect(() => {
    ...
    const handleResizeMove = (e: MouseEvent) => {
      ...
      const newHeight = Math.max(
        minHeight,
        resizingSection.startHeight + deltaY,
      );
  ...
  const getSelectedItemData = () => {
    ...
    for (const section of sections) {
      if (section.id === selectedItem.sectionId) {
        const items =
          selectedItem.type === "table" ? section.tables : section.bars;
    ...
    <div className="space-y-4">
      ...
      <div
        ...
        style={{
          ...
          backgroundImage:
            "radial-gradient(circle, #a1a1aa40 1px, transparent 1px)",
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="h-6 w-6"
                  onClick={() =>
                    setEditingName({ type: "section", sectionId: section.id })
                  }
                >
            ...
            {section.tables.map((table) => (
              <div
                ...
                style={{
                  ...
                  top: table.y,
                  opacity:
                    isDragging && draggedItem?.itemId === table.id ? 0.7 : 1,
                  userSelect: "none",
                }}
              >
                <div
            ...
            {section.bars.map((bar) => (
              <div
                ...
                style={{
                  ...
                  top: bar.y,
                  opacity:
                    isDragging && draggedItem?.itemId === bar.id ? 0.7 : 1,
                  userSelect: "none",
                }}
              >
                <div
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-foreground mb-4">
                {language === "es"
                  ? "No hay secciones. Crea una para comenzar."
                  : "No sections. Create one to start."}
              </p>
              <GlowButton onClick={addSection}>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {selectedItemData ? translateName(selectedItemData.name) : ""}
            </DialogTitle>
            <DialogDescription>
              {selectedItemData && (
                <Badge
                  className={`${statusColors[selectedItemData.status].replace("from-", "bg-").split(" ")[0]}`}
                >
              ...
              <div className="flex gap-2 pt-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    setSelectedTableForOrder(
                      `${selectedItem.sectionId}|${selectedItem.itemId}|${selectedItem.type}`,
                    );
                  }}
                >
                  <Plus className="w-4 h-4 mr-2" />
            ...
            <div className="space-y-4">
              ...
              {selectedReservation.status === "confirmed" && (
                ...
                >
                  ...
                  {language === "es"
                    ? "¡Llegó! Sentar Cliente"
                    : "Arrived! Seat Customer"}
                </Button>
              )}
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-4">
              ...
              <Button
                className="flex-1"
                disabled={
                  !newReservation.customer_name || !newReservation.table_id
                }
                onClick={async () => {
                  const supabase = createClient();
