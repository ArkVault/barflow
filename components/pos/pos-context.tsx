...
interface POSContextValue {
  ...
  selectedAccountForOrder: string | null;
  setSelectedAccountForOrder: React.Dispatch<
  ...
  setActiveTab: React.Dispatch<
    React.SetStateAction<"mesas" | "comandas" | "historial">
  >;
}
...
export function POSProvider({ children }: POSProviderProps) {
  ...
  const [selectedTableForOrder, setSelectedTableForOrder] = useState<
    string | null
  >(null);
  const [selectedAccountForOrder, setSelectedAccountForOrder] = useState<
    string | null
  >(null);
  ...
  const fetchProducts = async () => {
    ...
    try {
      const result = await fetchPosProductsUseCase(
  ...
  const refreshSales = useCallback(async () => {
    ...
    try {
      const loadedSales = await fetchPosSalesUseCase(
        operationsRepository,
        establishmentId,
      );
  ...
  const openNewAccount = useCallback(
    (sectionId: string, itemId: string, type: "table" | "bar") => {
      setSections((prev) =>
        openNewAccountInSections(prev, sectionId, itemId, type),
      );
    },
    [],
  );
  ...
  const closeAccount = useCallback(
    async (
      ...
      const closeCandidate = findAccountToClose(
        sections,
        sectionId,
        itemId,
        accountId,
      ...
      if (!closeCandidate || closeCandidate.account.items.length === 0) {
        ...
        setSections((prev) =>
          closeAccountInSections(prev, sectionId, itemId, accountId, type),
        );
        return;
      }
  ...
  const cancelAccount = useCallback(
    (
      ...
      setSections((prev) =>
        cancelAccountInSections(prev, sectionId, itemId, accountId, type),
      );
  ...
  const removeItemFromAccount = useCallback(
    (
      sectionId: string,
      itemId: string,
      accountId: string,
    ...
    for (const section of sections) {
      if (section.id === selectedItem.sectionId) {
        ...
        if (item && item.currentAccountId) {
          return (
            item.accounts.find((a) => a.id === item.currentAccountId) || null
          );
        }
      }
    }
