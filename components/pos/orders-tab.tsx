...
export function OrdersTab() {
  ...
  // Get accounts for the currently selected table/bar
  const selectedTableAccounts = (() => {
    if (!selectedTableForOrder) return [];
    const [sectionId, itemId, type] = selectedTableForOrder.split("|");
    const section = sections.find((s) => s.id === sectionId);
    if (!section) return [];
    const items = type === "bar" ? section.bars : section.tables;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <Card className="neumorphic border-0 p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              ...
              <Select
                ...
                onValueChange={(val) => {
                  setSelectedTableForOrder(val);
                  setSelectedAccountForOrder(null);
                }}
              >
          ...
          {/* Account Selection */}
          {selectedTableForOrder && selectedTableAccounts.length > 0 && (
            <div className="mt-3">
              <label className="text-xs font-medium text-muted-foreground mb-1 block">
                {language === "es" ? "Seleccionar Cuenta" : "Select Account"}
              </label>
              <Select
                value={selectedAccountForOrder || ""}
                onValueChange={setSelectedAccountForOrder}
              >
                <SelectTrigger className="w-full">
                  <SelectValue
                    placeholder={
                ...
                <SelectContent>
                  {selectedTableAccounts.map((account) => (
        ...
        <Card className="neumorphic border-0 p-4 sticky top-4">
          ...
          {currentOrder.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              <p className="text-sm">
                {language === "es" ? "Orden vacía" : "Empty order"}
              </p>
              ...
              <div className="border-t pt-4 space-y-3">
                ...
                {!selectedTableForOrder && (
                  <p className="text-xs text-center text-muted-foreground">
                    {language === "es"
                      ? "Selecciona una mesa o barra primero"
                      : "Select a table or bar first"}
                  </p>
                )}
              </div>
            </>
