...
import { Plus, Check, Trash2, Crown, Layers } from "lucide-react";
...
export function MenuManager({
  ...
  const loadMenus = async () => {
    ...
    try {
      ...
      console.log(
        "MenuManager - Loaded from Supabase:",
        data?.length || 0,
        "menus",
      );
      console.log(
        "MenuManager - Menu details:",
        data?.map((m) => ({
          id: m.id,
          name: m.name,
          active: m.is_active,
          secondary: m.is_secondary_active,
        })),
      );
      ...
      onActiveMenusChange?.(activeMenu?.id || null, secondaryMenu?.id || null);
    } catch (error: any) {
  ...
  const createMenu = async () => {
    if (!newMenuName.trim()) {
      toast.error(
        language === "es"
          ? "Por favor ingresa un nombre para el menú"
    ...
    try {
      ...
      toast.error(
        language === "es" ? "Error al crear menú" : "Error creating menu",
      );
    }
  };
  ...
  const activateMenu = async (menuId: string) => {
    try {
      ...
      toast.success(
        language === "es"
          ? "Menú activado como principal"
          : "Menu activated as primary",
      );
      ...
      toast.error(
        language === "es" ? "Error al activar menú" : "Error activating menu",
      );
    }
  };
  ...
  const activateSecondaryMenu = async (menuId: string) => {
    ...
    try {
      ...
      toast.success(
        language === "es"
          ? "Menú activado como secundario"
          : "Menu activated as secondary",
      );
      loadMenus();
    } catch (error: any) {
      ...
      toast.error(
        language === "es"
          ? "Error al activar menú secundario"
          : "Error activating secondary menu",
      );
    }
  };
  ...
  const deactivateSecondaryMenu = async (menuId: string) => {
    try {
      ...
      toast.success(
        language === "es"
          ? "Menú secundario desactivado"
          : "Secondary menu deactivated",
      );
      loadMenus();
    } catch (error: any) {
      ...
      toast.error(
        language === "es"
          ? "Error al desactivar menú secundario"
          : "Error deactivating secondary menu",
      );
    }
  };
  ...
  const deleteMenu = async (menuId: string) => {
    ...
    if (menu?.is_active) {
      toast.error(
        language === "es"
          ? "No puedes eliminar el menú activo"
          : "You cannot delete the active menu",
      );
      return;
    }
    if (menu?.is_secondary_active) {
      toast.error(
        language === "es"
          ? "No puedes eliminar el menú secundario activo"
          : "You cannot delete the active secondary menu",
      );
      return;
    }
    ...
    try {
      ...
      toast.error(
        language === "es" ? "Error al eliminar menú" : "Error deleting menu",
      );
    }
  };
  ...
  const inactiveMenus = menus.filter(
    (m) => !m.is_active && !m.is_secondary_active,
  );

  console.log("MenuManager - All menus:", menus.length);
  console.log("MenuManager - Active menu:", activeMenu?.name, activeMenu?.id);
  console.log(
    "MenuManager - Secondary active menu:",
    secondaryActiveMenu?.name,
    secondaryActiveMenu?.id,
  );
  console.log(
    "MenuManager - Inactive menus:",
    inactiveMenus.map((m) => `${m.name} (${m.id})`),
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <GlowButton onClick={() => setShowCreateDialog(true)}>
          ...
          <span className="hidden sm:inline">
            {language === "es" ? "Nuevo Menú" : "New Menu"}
          </span>
        </GlowButton>
      </div>
            ...
            {secondaryActiveMenu && (
              ...
              >
                ...
                >
                  ...
                  <div className="flex justify-end mb-1">
                    <span
                      className="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full text-white shadow-lg"
                      style={{
                        background:
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {new Date(
                        secondaryActiveMenu.created_at,
              ...
              >
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1.5 items-center">
                    ...
                    >
                      <div className="w-4 h-4 rounded-full bg-gradient-to-br from-blue-300 to-blue-400 flex items-center justify-center shadow-inner">
                    ...
                    {activeMenu && (
                      ...
                      >
                        <div className="w-4 h-4 rounded-full bg-gradient-to-br from-slate-300 to-slate-400 flex items-center justify-center shadow-inner">
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {language === "es" ? "Crear Nuevo Menú" : "Create New Menu"}
            </DialogTitle>
            <DialogDescription>
              {language === "es"
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="menu-name">
                {language === "es" ? "Nombre del Menú" : "Menu Name"}
              </Label>
              <Input
                id="menu-name"
                value={newMenuName}
          ...
          <DialogFooter>
            ...
            <Button onClick={createMenu}>
              {language === "es" ? "Crear Menú" : "Create Menu"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
