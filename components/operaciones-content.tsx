...
export function OperacionesContent() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      console.log(
        "OperacionesPage - Fetching products from menu IDs:",
        menuIds,
      );
  ...
  const sendOrderToTable = () => {
    ...
    for (const section of sections) {
      const table = section.tables.find(
        (t) => `${section.id}-${t.id}` === selectedTableForOrder,
      );
      if (table) {
      ...
      const bar = section.bars.find(
        (b) => `${section.id}-${b.id}` === selectedTableForOrder,
      );
      if (bar) {
        targetSection = section;
        targetBar = bar;
  ...
  const closeAccount = async (
    ...
    if (accountToClose && establishmentId) {
      try {
        ...
        toast.error(
          language === "es" ? "Error al guardar la venta" : "Error saving sale",
        );
      }
    } else {
      if (!accountToClose) {
      ...
      if (!establishmentId) {
        ...
        toast.error(
          language === "es"
            ...
            : "Could not identify establishment",
        );
      }
    }
                ...
                if (table.id === itemId) {
                  const updatedAccounts = table.accounts.map((acc) =>
                    acc.id === accountId
                      ? {
                          ...acc,
                          status: "pagada" as AccountStatus,
                          closedAt: new Date(),
                        }
                      : acc,
                  );
  ...
  const getCurrentAccount = (): Account | null => {
    ...
    if (selectedItem.type === "table") {
      ...
      return (
        table.accounts.find((acc) => acc.id === table.currentAccountId) || null
      );
    } else {
      const bar = section.bars.find((b) => b.id === selectedItem.itemId);
      if (!bar || !bar.currentAccountId) return null;
      return (
        bar.accounts.find((acc) => acc.id === bar.currentAccountId) || null
      );
    }
  };
      ...
      <div className="mb-6">
        <div className="inline-flex items-center gap-3 p-1.5 w-fit">
          <button
            ...
            className={`px-8 py-3.5 rounded-3xl transition-all flex items-center gap-2.5 text-base font-medium ${
              activeTab === "mesas"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {loadingProducts ? (
                ...
                getFilteredProducts().map((product) => (
                  <div key={product.id} className="neumorphic rounded-2xl p-4">
          ...
          <div className="space-y-4">
            <div className="neumorphic rounded-2xl p-4 sticky top-4">
              <h3 className="text-lg font-bold mb-4">
                {language === "es" ? "Comanda Actual" : "Current Order"}
              </h3>
                ...
                <>
                  ...
                  <div className="border-t pt-4 space-y-3">
                    ...
                    <GlowButton
                      ...
                      className="w-full"
                      disabled={
                        !selectedTableForOrder || currentOrder.length === 0
                      }
                    >
          ...
          >
            {sections.map((section) => (
              <div
                key={section.id}
                onMouseDown={(e) =>
                  handleMouseDown(e, "section", section.id, section.id)
                }
              ...
              >
                ...
                <div className="absolute -top-10 left-0 right-0 flex items-center justify-between px-2">
                  {editingName?.type === "section" &&
                  editingName.sectionId === section.id ? (
                    <Input
                      value={section.name}
                      onChange={(e) =>
                        setSections(
                ...
                {section.tables.map((table) => (
                  <div
                    key={table.id}
                    onMouseDown={(e) =>
                      handleMouseDown(e, "table", section.id, table.id)
                    }
                    ...
                    style={{
                      ...
                      top: table.y,
                      opacity:
                        isDragging && draggedItem?.itemId === table.id
                          ? 0.5
                          : 1,
                      userSelect: "none",
                    }}
                ...
                {section.bars.map((bar) => (
                  <div
                    key={bar.id}
                    onMouseDown={(e) =>
                      handleMouseDown(e, "bar", section.id, bar.id)
                    }
                    className="absolute cursor-move group"
                    style={{
                      left: bar.x,
                      top: bar.y,
                      opacity:
                        isDragging && draggedItem?.itemId === bar.id ? 0.5 : 1,
                      userSelect: "none",
                    }}
                  >
                    <div
                      ...
                      style={{
                        width:
                          bar.orientation === "vertical" ? "72px" : "180px",
                        height:
                          bar.orientation === "vertical" ? "180px" : "72px",
                ...
                >
                  <div
                    className="absolute bottom-2 right-2 w-5 h-5 border-r-3 border-b-3 border-primary/50 group-hover/resize:border-primary transition-colors rounded-br-lg"
                    style={{
            ...
            <DialogTitle className="text-2xl font-bold">
              {selectedItem &&
                sections
                  ...
                  ?.tables.find((t) => t.id === selectedItem.itemId)?.name}
              {selectedItem &&
                sections
                  .find((s) => s.id === selectedItem.sectionId)
                  ?.bars.find((b) => b.id === selectedItem.itemId)?.name}
            </DialogTitle>
            <DialogDescription>
                ...
                <div className="neumorphic rounded-2xl p-4">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="font-medium">
                      {currentAccount.openedAt.toLocaleTimeString(
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="font-medium">
                      {getElapsedTime(currentAccount.openedAt)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between pt-3 border-t">
                    ...
                    <span className="text-2xl font-bold text-primary">
                      ${currentAccount.total.toFixed(2)}
                    </span>
                  </div>
                </div>
