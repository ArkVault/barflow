...
export function AccountModal({ isOpen, onClose }: AccountModalProps) {
  ...
  const getPlanName = () => {
    switch (subscription.planType) {
      case "starter_monthly":
        return "Starter (Mensual)";
      case "starter_yearly":
        return "Starter (Anual)";
      case "business_monthly":
        return "Business (Mensual)";
      case "business_yearly":
        return "Business (Anual)";
      case "chain":
        return "Cadena";
      case "enterprise":
        return "Enterprise";
  ...
  // Self-serve plans (Cadena & Enterprise are quote-only)
  const plans = [
    {
      id: "starter_monthly",
      name: "Starter",
      price: `$${PLAN_PRICING.starter.monthly.toLocaleString("es-MX")}`,
      period: "/mes",
      description: "1 sucursal · hasta 5 usuarios",
      features: ["1 sucursal", "Hasta 5 usuarios", "Inventario ilimitado"],
      priceId: process.env.NEXT_PUBLIC_STRIPE_STARTER_MONTHLY_PRICE_ID ?? "",
    },
    {
      id: "starter_yearly",
      name: "Starter Anual",
      price: `$${PLAN_PRICING.starter.yearly.toLocaleString("es-MX")}`,
      period: "/mes",
      description: "Ahorra $4,800 al año",
      features: ["1 sucursal", "Hasta 5 usuarios", "2 meses gratis"],
    ...
    },
    {
      id: "business_monthly",
      name: "Business",
      price: `$${PLAN_PRICING.business.monthly.toLocaleString("es-MX")}`,
      period: "/mes",
      description: "1 sucursal · hasta 10 usuarios · IA",
      features: [
        "1 sucursal",
        "Hasta 10 usuarios",
    ...
    },
    {
      id: "business_yearly",
      name: "Business Anual",
      price: `$${PLAN_PRICING.business.yearly.toLocaleString("es-MX")}`,
      period: "/mes",
      description: "Ahorra $6,000 al año",
      features: [
        "1 sucursal",
        "Hasta 10 usuarios",
        "Protección de inventario",
  ...
  const isUpgrade = (planId: string) => {
    const current = tierOrder[subscription.planType ?? "free_trial"] ?? 0;
    const target = tierOrder[planId] ?? 0;
      ...
      >
        ...
        <div className="flex border-b border-white/5 px-6">
          <button
            ...
            className={`relative mr-6 py-4 text-sm font-medium transition-colors ${
              activeTab === "profile"
                ? "text-white"
              ...
              {currentPlanIsTopTier && (
                <div className="rounded-xl border border-white/5 bg-white/[0.02] p-6 text-center">
                  ...
                  <p className="text-sm text-white/40 mt-1">
                    Tienes acceso a todas las funcionalidades exclusivas de
                    BarFlow.
                  </p>
                </div>
              )}
